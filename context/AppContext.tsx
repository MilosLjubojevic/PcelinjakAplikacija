...
import { scheduleInspectionNotifications } from "../utils/notifications";
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [state, setState] = useState<AppState>(
    DEV_MODE ? generateMockData() : emptyState,
  );
  const [loading, setLoading] = useState<boolean>(DEV_MODE ? false : true);
  ...
  // Reschedule inspection notifications whenever locations change
  useEffect(() => {
    if (state.locations.length === 0) return;
    scheduleInspectionNotifications(state.locations).catch(() => {});
  }, [state.locations]);
      ...
      if (!migrated) {
        ...
        if (oldData) {
          try {
            ...
            console.error(
              "Migration error (continuing with Supabase fetch):",
              migrationError,
            );
  ...
  useEffect(() => {
    if (DEV_MODE || !userId || !isSupabaseConfigured()) return;
  ...
  const metrics = useMemo<DashboardMetrics>(() => {
    const allItems = state.locations.flatMap((loc) =>
    ...
    const hivesNeedingAttention = hiveItems.filter(
      (h) => h.health === "bad",
    ).length;
    ...
    const nucleiForSale = swarmItems.filter(
      (s) => s.swarmStatus === "ready",
    ).length;
  ...
  const updateLocation = useCallback(
    async (id: string, updates: Partial<Location>) => {
      ...
      if (updates.rows) {
        dbOps.push(
          db.syncLocationChildren(
            userId,
            id,
            currentLocation.rows,
            updates.rows,
          ),
        );
      }
      ...
      if (updates.description !== undefined)
        scalarUpdates.description = updates.description;
  ...
  const addQueen = useCallback(
    async (queen: Queen) => {
      ...
      if (!success) {
        updateState((prev) => ({
          ...prev,
          queens: prev.queens.filter((q) => q.id !== queen.id),
        }));
  ...
  const updateQueen = useCallback(
    async (id: string, updates: Partial<Queen>) => {
      if (!userId) return;
      ...
      updateState((prev) => ({
        ...
        queens: prev.queens.map((queen) =>
          queen.id === id
            ? { ...queen, ...updates, updatedAt: new Date() }
            : queen,
        ),
      }));
      if (DEV_MODE) return;
  ...
  const addQueenBoxRow = useCallback(
    async (row: QueenBoxRow) => {
      if (!userId) return;
      updateState((prev) => ({
        ...prev,
      ...
      if (!success) {
        updateState((prev) => ({
          ...prev,
          queenBoxRows: (prev.queenBoxRows || []).filter(
            (r) => r.id !== row.id,
          ),
        }));
  ...
  const updateQueenBoxRow = useCallback(
    async (id: string, updates: Partial<QueenBoxRow>) => {
      if (!userId) return;
      ...
      if (updates.queenBoxes) {
        dbOps.push(
          db.syncQueenBoxRowChildren(
            userId,
            id,
            currentRow.queenBoxes,
            updates.queenBoxes,
          ),
        );
      }
      ...
      if (updates.locationId !== undefined)
        scalarUpdates.locationId = updates.locationId;
      if (updates.capacity !== undefined)
        scalarUpdates.capacity = updates.capacity;
  ...
  const reorderQueenBoxRows = useCallback(
    async (reordered: QueenBoxRow[]) => {
      if (!userId) return;
      const snapshot = state.queenBoxRows || [];
      ...
      const results = await Promise.all(
        withOrder
          .filter(
            (row, i) => snapshot.find((r) => r.id === row.id)?.order !== i,
          )
          .map((row) =>
            db.updateQueenBoxRowScalars(userId, row.id, { order: row.order }),
          ),
      );
      if (results.some((ok) => !ok)) {
        ...
        showToast(
          "Neuspješno preuređivanje redova. Podaci su osvježeni.",
          "error",
        );
      }
    },
  ...
  const deleteQueenBoxRow = useCallback(
    async (id: string) => {
      if (!userId) return;
      const snapshot = state.queenBoxRows || [];
      updateState((prev) => ({
  ...
  const addIncome = useCallback(
    async (income: Income) => {
      ...
      if (!success) {
        updateState((prev) => ({
          ...prev,
          incomes: (prev.incomes || []).filter((i) => i.id !== income.id),
        }));
  ...
  const addPolenHarvest = useCallback(
    async (harvest: PolenHarvest) => {
      if (!userId) return;
      updateState((prev) => ({
        ...prev,
        polenHarvests: [...(prev.polenHarvests || []), harvest],
      }));
      if (DEV_MODE) {
      ...
      if (!success) {
        updateState((prev) => ({
          ...prev,
          polenHarvests: (prev.polenHarvests || []).filter(
            (h) => h.id !== harvest.id,
          ),
        }));
  ...
  const updatePolenHarvest = useCallback(
    async (id: string, updates: Partial<PolenHarvest>) => {
      if (!userId) return;
  ...
  const deletePolenHarvest = useCallback(
    async (id: string) => {
      if (!userId) return;
      const snapshot = state.polenHarvests || [];
      updateState((prev) => ({
        ...prev,
        polenHarvests: (prev.polenHarvests || []).filter((h) => h.id !== id),
      }));
      if (DEV_MODE) {
